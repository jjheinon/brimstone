...
void main()
{
    float currTime = currentTimeUniform;
				...
				//slope=1.5-smoothstep(0.55,0.0,slope);
				
    
				float noise=abs(fbm(uv*1.));
				slope=(noise*0.2)+(slope-((1.0-noise)*slope*0.1))*0.2;
				slope=clamp(slope,0.0,0.4);
				...
				Snowout = 0.05+(slope*0.2/*(suns+0.3)*/)/*+(sunsh*0.6)*/+N1+N2+N3+N4+N5;
