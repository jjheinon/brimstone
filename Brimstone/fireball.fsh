...
float distanceFunc(vec3 p)
{
    ...
    d += fbm(p*_NoiseFreq + _NoiseAnim*u_time) * _NoiseAmp;
    return d;
}
...
vec4 gradient(float x)
{
    ...
    const vec4 c2 = vec4(2, 0.5, 0, 0); 	// black
    const vec4 c3 = vec4(1, 0.5, 0, 0.5); 	// blue
    const vec4 c4 = vec4(2, 2, 1, 0); 	// black
